...
class Registration_form extends PieReg_Base
{
	...
	function validateRegistration($errors)
	{
		...
		*/
		if( $this->piereg_pro_is_activate )	
		{
			...
			if( isset($piereg['piereg_blk_email']) && (isset($piereg['enable_blockedemail']) && $piereg['enable_blockedemail'] == 1) )
			{
				...
				if(	$this->isEmailAddressIsBlocked($this->pie_post_array['e_mail'],$array_emailaddr) ) {
		...
		*/
			...
			else if($field['type']=="invitation"  && $piereg["enable_invitation_codes"]=="1")
			{
				...
				if($required != "" || $this->pie_post_array['invitation'] != "")
				{
					...
					if($this->piereg_pro_is_activate){
						...
						$_invitation_code_key = "";
						if(isset($this->pie_post_array['invitation_key']) && !empty($this->pie_post_array['invitation_key']))
						{
							$_invitation_code_key = $this->pie_post_array['invitation_key'];
							if(!get_option('pie_invitation_link_'.$_invitation_code_key))
							{
								$errors->add( $slug , apply_filters("piereg_invalid_invitation_link",'<strong>'.ucwords(__('error','pie-register')).'</strong>: '.__('Invitation code link has been expired','pie-register' )));
							}
						}
					}
		...
		if($this->piereg_pro_is_activate && (isset($_invitation_code_key) && !empty($_invitation_code_key)) && sizeof($errors->errors) == 0)
		{
			delete_option('pie_invitation_link_'.$_invitation_code_key);
		}
		return $errors;
	}
