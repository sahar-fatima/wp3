...
class Profile_admin extends PieReg_Base
{
    ...
    function addProfilePic($disabled)
	{
		...
		$data .= '<br /><img src="'.$imgPath.'" style="max-width:150px;" alt="'.__($imgPath,"pie-register").'" />';
		echo $data;
	}
    ...
    function addDate($disabled)
    {
        ...
        if ($this->field['date_type'] == "datefield") {
            ...
            for ($a = 1; $a <= 12; $a++) {
                ...
                echo '<option ' . $sel . ' value="' . $a . '">' . __(sprintf("%02s", $a),"pie-register") . '</option>';
            }
            echo '</select>
	...
	function addList($disabled)
	{
		...
		for($a = 1 ,$c=0; $a <= $this->field['rows'] ; $a++,$c++)
		{
			...
			{
				echo '<input'.$disabled.' value="'.((isset($list[$c][$d]))?$list[$c][$d]:"").'" style="width:'.$width.'%;margin-right:2px;" type="text" name="'.$this->slug.'['.$c.'][]" class="input_fields"> ';
			}
			echo '</div>';
		}
	}
    ...
    function edit_user_profile($user)
    {
        ...
        if (sizeof($this->data) > 0 && is_array($this->data))
		{
			...
			if ( current_user_can( 'administrator' ) && $this->piereg_pro_is_activate === true )
			{
				echo '<tr>';
					echo '<td>'.__('Assign user to form: ','pie-register').'</td>';
					echo '<td>';
					$st 	 = '<select name="pie_form_assign"><option value="">%s</option>%s</select>';
						...
						for($a=1;$a<=$pie_forms_count;$a++)
						{
							...
							{
								$selected		= (isset($form_id) && $pie_form_info['Id'] == $form_id )? "selected" : "";
								$dropdown_options		.= '<option ' . $selected . ' value="'.$pie_form_info['Id'].'">'.$pie_form_info['Title'].'</option>';
							}
						}
										
						// combine template and options
						$select = sprintf( $st, __( 'Select Pie Register Form' ), $dropdown_options );
			...
			*/
            ...
            if ((!isset($field_name) || empty($field_name)) && $required) {
                ...
                if (!is_numeric($field_name)) {
                    $errors->add( $slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.$this->field['label'] .apply_filters("piereg_field_must_contain_only_numbers",__(' field must contain only numbers.','pie-register' )));
			    }
            } else if ($rule == "alphanumeric") {
                if (!preg_match("/^([a-z 0-9])+$/i", $field_name)) {
                   	$errors->add( $slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.$this->field['label'] .apply_filters("piereg_field_may_only_contain_alpha_numeric_characters",__(' field may only contain alpha-numeric characters.','pie-register' )));
                }
            } else if ($rule == "email") {
                if (!filter_var($field_name, FILTER_VALIDATE_EMAIL)) {
                    $errors->add( $slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.$this->field['label'].apply_filters("piereg_field_must_contain_a_valid_email_address",__(' field must contain a valid email address.','pie-register' )));
                }
            } else if ($rule == "website") {
                if (!filter_var($field_name, FILTER_VALIDATE_URL)) {                   
					$errors->add( $slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.$this->field['label'] .apply_filters("piereg_must_be_a_valid_URL",__(' must be a valid URL.','pie-register' )));
                }
            }
        }
