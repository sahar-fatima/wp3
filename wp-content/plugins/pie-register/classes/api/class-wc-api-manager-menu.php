...
class Piereg_API_Manager_Example_MENU{
	...
	public function wc_am_license_key_deactivation( $input, $addon = false ) {
		...
		if ( ( $options == 'on' && $activation_status == 'Activated' && $api_key != '' && $api_email != '' ) || $addon['is_piereg_pro_inactive'] === true ) {
			
			if($addon){
				
				if( $addon['is_piereg_pro_inactive'] == true )
				{
					...
					$args = array(
									'email' 					=> $api_email,
									'licence_key' 				=> $api_key,
									'is_piereg_pro_inactive'	=> $addon['is_piereg_pro_inactive']
								);
				}
