...
class PieRedirectSettings extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        foreach ( $posts_array as $key => $post )
        {
			...
			//Order By
			$orderby = ((isset($_GET['orderby']) && !empty($_GET['orderby']))?"&orderby=".esc_attr($_GET['orderby']):"");
			//Ordered
			$order = ((isset($_GET['order']) && !empty($_GET['order']))?"&order=".esc_attr($_GET['order']):"");
			//User Role
			$posts[ $key ]->user_role = '<span><a href="'.admin_url("admin.php?page=pie-settings&tab=pages&subtab=role-based&action=edit&pie_id=".$posts[ $key ]->id.$page_id.$orderby.$order).'" title="'.__("Click here to edit","pie-register").'">'.$user_role_value.'</a></span>';
			//Logged In URL
			...
			//Status
			$posts[ $key ]->status = '<a herf="javascript:;" onclick="changeStatus(\''.$post->id.'\',\''.$user_role_value.'\',\''.$title.'\');" title="'.__($title,"pie-register").'" class="'.$class.'"></a>';
			$posts[ $key ]->status = '<a class="delete" href="javascript:;" onclick="confirmDel(\''.$post->id.'\',\''.$user_role_value.'\');" title="'.__("Delete","pie-register").'"></a>';
