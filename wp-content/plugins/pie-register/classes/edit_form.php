...
class Edit_form extends PieReg_Base
{
	...
	function validateRegistration($errors)
	{
		...
		*/
		...
		{
			$errors->add( "password" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_old_password_Fields_do_not_match",__('Old Password Field do not match!','pie-register' )));
		}elseif(!empty($this->pie_post_array['password']) && empty($this->pie_post_array['old_password']) ){
			$errors->add( "password" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_old_password_Fields_do_not_match",__('Invalid Old Password Field!','pie-register' )));
		}
		...
		{
			$errors->add( "password" , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.apply_filters("piereg_old_password_and_new_password_same",__('Old and New passwords are same!','pie-register' )));
		}
		...
		foreach($this->data as $field)
		 {
				...
				if($this->user->data->user_email != $this->pie_post_array['e_mail'] && !empty($global_options['email_edit_verification_step']) )
				{
					...
					if(!email_exists($this->pie_post_array['e_mail']))
					{
						...
						*/
						...
						$errors->add( $slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.__('This Email already Exists','pie-register') );
					}
				}else{
			...
			*/
			...
			else if($rule=="number" && !empty($field_name))
			{
				...
				{
					$errors->add( $slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.$field['label'] .apply_filters("piereg_field_must_contain_only_numbers",__(' field must contain only numbers.','pie-register' )));
				}	
			}
			else if($rule=="alphanumeric" && !empty($field_name))
			{
				...
				{
					$errors->add( $slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.$field['label'] .apply_filters("piereg_field_may_only_contain_alpha_numeric_characters",__(' field may only contain alpha-numeric characters.','pie-register' )));
				}	
			}		
			else if($rule=="alphabetic" && !empty($field_name))
			{
				...
				{
					$errors->add( $slug ,"<strong>". __(ucwords("Error"),"pie-register").":</strong> ".$field['label'] .apply_filters("piereg_field_may_only_contain_alphabetic_characters",__(" field may only contains alphabetic letters."  ,"pie-register")));		
				}	
			}
			...
			if( (isset($phone_format) && $phone_format == "international") && isset($this->pie_post_array[$slug]) && !empty($this->pie_post_array[$slug]) ){
				if(!is_numeric($field_name)){
					$errors->add( $slug ,"<strong>". __(ucwords("Error"),"pie-register").":</strong> ".$field['label'] .apply_filters("piereg_must_be_a_valid_URL",__(" invalid field." ,"pie-register")));
				}
			}
		 }	
	...
	function UpdateUser()
	{
		...
		if($this->pie_error)
			$this->pie_error_msg = __('Something Went Wrong updating Profile fields, please try again!','pie-register');
		if($this->pie_success)
