		...
		if (isset($pie_register_base->pie_post_array['reset_pass']) && trim($pie_register_base->pie_post_array['user_login']) != "")
		{
			...
			if( $error_found == 0 && (isset($error) && count($error) == 0) ){
				...
				elseif( email_exists($username) ){
					...
					$error[] = '<strong>'.ucwords(__("error","pie-register")).'</strong>: '.__('Username or Email was not found, try again!','pie-register');
				}
				if ($user_exists){
					...
					if (count($error) == 0 )
					{
						...
						$success =  '<strong>'.ucwords(__("success","pie-register")).'</strong>: '.apply_filters("piereg_message_will_be_sent_to_your_email",__('A message will be sent to your email address.','pie-register'));
					}	
				}
			}
		}
