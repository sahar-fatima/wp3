...
<form method="post" action="" id="piereg_form_general_settings_page" onsubmit="return validateSettingsSecurity();">
  ...
  <div class="fields">
    ...
    <div class="radio_fields">
      ...
      <?php if($this->piereg_pro_is_activate) { ?>
          <label for="captcha_in_login_attempts" class="lbl_style1"> <?php _e('After', 'pie-register'); ?>
            ...
            <?php _e('invalid login attempts', 'pie-register'); ?></label>
      <?php } ?>
    </div>
  </div>
  ...
  <div class="fields piereg_captcha_type_show" <?php echo ((!isset($piereg['captcha_in_login_value']) || isset($piereg['captcha_in_login_value']) && $piereg['captcha_in_login_value'] == 0 )?'style="display:none;"':'') ?>>
    ...
    <?php _e("Appear Captcha in login form which you want.",'pie-register') ?>
    </span>
  ...
  <div class="fields piereg_captcha_forgot_pass_type_show" <?php echo (!isset($piereg['captcha_in_forgot_value']) || (isset($piereg['captcha_in_forgot_value']) && $piereg['captcha_in_forgot_value'] == 0 )?'style="display:none;"':'') ?>>
    ...
    <span class="quotation">
    <?php _e("Appear Captcha in Forgot Password form which you want.",'pie-register') ?>
    </span>
  ...
  <div class="fields">
    <p>
      <?php _e("Pie Register integrates with reCAPTCHA, a free CAPTCHA services that helps to digitize Books while Protecting your forms from spam bots. Please click <a href='https://www.google.com/recaptcha/admin' target='_blank'>here</a> to get reCaptcha keys for your site.",'pie-register') ?>
    </p>
  ...
  <div class="fields">
   <p>
      <?php _e("Note: Admin and Email verifications wont work when Payment gateway is enable.",'pie-register') ?>
    </p>
  </div>
  <div class="fields">
    <label>
    ...
    <div>
      <select name="verification" id="verification_2" >
        ...
        <?php _e("Admin Approval AND Verify Email Address","pie-register"); echo ($this->piereg_pro_is_activate)?'': __(" - [PRO]","pie-register"); ?>
        </option>
      </select>
    </div>
    <div class="verification_data"> <span>
      ...
      <span>
      <?php _e("<strong>Verify Email Address</strong> - Require new registrations to click a link sent via email to enable their account.",'pie-register') ?>
      </span>
      <p><strong>
      ...
      <p>
        <?php _e("User Ids that are not verified within the grace period are deleted at next logon attempt. 0 (Zero) for Unlimited",'pie-register') ?>
      </p>
    </div>
  </div>
  <div class="fields">
    ...
    <div class="radio_fields max_label_300">
      ...
      <label for="email_edit_verification_2">
        <?php _e("<strong>2-Step:</strong> Authenticate request by sending an email to old address + verify new address.",'pie-register') ?>
      </label>
  ...
  <hr class="seperator" />
  <h3>
    <?php _e("Restrict Website Content For Search Engine(s) / Bot",'pie-register'); ?>
  </h3>
  <div class="fields">
    <div class="radio_fields">
