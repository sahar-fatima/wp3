...
<div id="container"  class="pieregister-admin">
    ...
    if( isset($this->pie_post_array['error']) && !empty($this->pie_post_array['error']) ){
        echo '<div id="error" class="error fade"><p><strong>' . $this->pie_post_array['error'] . '</strong></p></div>';
    }
    ...
    elseif( isset($this->pie_post_array['success']) && !empty($this->pie_post_array['success']) ){
        echo '<div id="message" class="updated fade"><p><strong>' . $this->pie_post_array['success'] . '</strong></p></div>';
	}
    ?>
    <div class="right_section">
        <div class="settings">
        	<?php if( $_GET['tab'] != 'license' ){ ?>
	        	<h2><?php _e("Licence Key Settings",'pie-register') ?></h2>
            <?php } ?>
            <div class="fields"> <?php _e("You need to activate the license key to use premium features",'pie-register') ?></div>
            <form method="post" action="">
				<?php if( function_exists( 'wp_nonce_field' )) wp_nonce_field( 'piereg_wp_license_key_nonce','piereg_license_key_nonce'); ?>
            	<?php if( $_GET['tab'] != 'license' ){ ?>
               	 	<h3><?php _e("Activation",'pie-register') ?></h3>
            	<?php } ?>
                <div class="fields">
                    <label for="piereg_license_key"><?php _e("License Key",'pie-register') ?></label>
                    ...
                    <span class="piereg_license_key_icon_span"><img class="piereg_license_key_icon" title="" src="<?php echo plugins_url("assets/images/warning.png", dirname(__FILE__) ); ?>"></span>
                </div>
                <div class="fields notice_license">
                	<span>Enter license key which is provided in your GT Store account under My Keys.</span>
                </div>
                <div class="fields">
                    <label for="piereg_license_email"><?php _e("Email Address",'pie-register') ?></label>
                ...
                <div class="fields fields2">
	                <input type="submit" class="submit_btn" name="save_license_key_settings" value=" <?php _e("Activate","pie-register");?> " />
                    <p><a href="https://store.genetech.co/" target="_blank" title="Purchase License Key"><?php _e("Already Purchased",'pie-register') ?>?</a> or <a href="http://pieregister.com/" target="_blank" title="Purchase License Key"><?php _e("Click here to Purchase",'pie-register') ?></a></p>
                </div>
