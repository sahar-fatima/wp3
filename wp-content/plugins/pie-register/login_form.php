...
$newpasspageLock = 0;
			...
			if(isset($_GET['payment']) && $_GET['payment'] == "success")
			{
				...
				$login_error = apply_filters("piereg_cancled_message",__("You canceled your payment.","pie-register"));
			}
			...
			elseif ( 'payment_cancel' == $_GET['action'] )
                $login_warning = '<strong>'.ucwords(__("warning","pie-register")).'</strong>: '.apply_filters("piereg_canelled_your_registration",__("You have canelled your registration.","pie-register"));

			elseif ( 'payment_success' == $_GET['action'] )
                $login_success = '<strong>'.ucwords(__("success","pie-register")).'</strong>: '.apply_filters("piereg_thank_you_for_registration",__("Thank you for your registration. You will receieve your login credentials soon.","pie-register"));		
				...
				if(sizeof($unverified )==1)
				{
					...
					if($user){
						...
						if(isset($user_meta[0]) && $user_meta[0] == 1){
							$login_warning = '<strong>'.ucwords(__("warning","pie-register")).'</strong>: '.apply_filters("piereg_canelled_your_registration",__("You are already activate","pie-register"));
							unset($user_meta);
							unset($user_name);
							unset($user);
						}
			...
			elseif ( 'resetpass' == $_GET['action'] || 'rp' == $_GET['action'] ){
				...
				if ( is_wp_error($user) ) {
					if ( $user->get_error_code() === 'expired_key' )
						$login_error = '<strong>'.ucwords(__("error","pie-register")).'</strong>: '.apply_filters("piereg_you_key_has_been_expired",__("You key has been expired, please reset password again!","pie-register").' <a href="'.pie_lostpassword_url().'" title="'.__("Password Lost and Found","pie-register").'">'.__("Lost your password?","pie-register").'</a>');
					else
						$login_error = '<strong>'.ucwords(__("error","pie-register")).'</strong>: '.apply_filters("piereg_this_reset_key_invalid_or_no_longer_exists",__("This Reset key is invalid or no longer exists. Please reset password again!","pie-register").' <a href="'.pie_lostpassword_url().'" title="'.__("Password Lost and Found","pie-register").'">'.__("Lost your password?","pie-register").'</a>');
						$newpasspageLock = 1;
				}else{
					$login_warning = '<strong>'.ucwords(__("warning","pie-register")).'</strong>: '.__('Enter your new password below.',"pie-register");
				}
				...
				if( isset($_POST['pass1'], $_POST['piereg_get_password_nonce']) && wp_verify_nonce( $_POST['piereg_get_password_nonce'], 'piereg_wp_get_password_nonce' ) && !is_wp_error($user)){
					...
					if(isset($_POST['pass1']) && trim($_POST['pass1']) == ""){
						...
						$login_error =  '<strong>'.ucwords(__("error","pie-register")).'</strong>: '.apply_filters("piereg_minimum_8_characters_required_in_password",__( 'Minimum 8 characters required in password',"pie-register" ));
						$errors->add( 'password_reset_mismatch',$login_error );
					...
					if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
						...
						$login_warning = '';
						$login_error = '';
						$login_success = '<strong>'.ucwords(__("success","pie-register")).'</strong>: '.apply_filters("piereg_your_password_has_been_reset",__( 'Your password has been reset.' , "pie-register"));
					}
				}
			}
        }
