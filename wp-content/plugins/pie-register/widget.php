...
class Pie_Register_Widget extends WP_Widget 
{
	function __construct() 
	{
		parent::__construct(
			...
			array( 'description' => __( 'Display Your Pie Register Registration Form on Sidebar', 'pie-register' ), ) // Args
		);		
	}
...
class Pie_Forgot_Widget extends WP_Widget 
{
	...
	public function widget( $args, $instance ) 
	{
		...
		if(is_user_logged_in() && $option['redirect_user']==1 ){
			...
			global $errors;
			$title = ($instance['title'])?$instance['title']:__( 'Forgot password', 'pie-register' );
			echo $args['before_widget'];
			if ( ! empty( $title ) )
