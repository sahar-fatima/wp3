...
/* 
Plugin Name: Pie Register - Custom Registration Form, Invitation based Registrations and User Login WordPress Plugin
	...
	class PieRegister extends PieReg_Base{
		...
		function __construct(){
			...
			*/
			...
			if( is_admin() && ( (isset($_POST['piereg_license_key']) && !empty($_POST['piereg_license_key'])) || $this->piereg_pro_is_activate || (isset($_POST['piereg_pro_not_activated'] )) ) )
			{
		...
		function pie_main(){
			...
			//var_dump($pie_plugin_db_version , PIEREG_DB_VERSION); die;
			...
			$this->is_pr_preview = (isset($_GET['pr_preview']) || isset($_GET['elementor-preview']) || isset($_GET['et_fb']) || (isset($_GET['preview']) && $_GET['preview'] == "true") )?true:false;
			...
			if( current_user_can( 'administrator' ) && $this->piereg_pro_is_activate ){
				// add notices for pro users here...	
			} else {
			...
			*/
			...
			if(isset($_GET['prfrmid']) and ((int)$_GET['prfrmid']) != 0 and isset($_GET['status']) and $_GET['status'] != "")
			{
				...
				$pr_form_option['Status'] 	= ( strtolower( trim( $_GET['status'] ) ) );
			...
			if( (isset($_POST['piereg_attachment_per_page_nonce']) && wp_verify_nonce( $_POST['piereg_attachment_per_page_nonce'], 'piereg_wp_attachment_per_page_nonce' )) && isset($_POST['pie_attachment_per_page_items']) and $_POST['pie_attachment_per_page_items'] != "")
			{
				$opt = get_option(OPTION_PIE_REGISTER);
				$val = ((int)($_POST['pie_attachment_per_page_items']) != 0)? ((int)$_POST['pie_attachment_per_page_items']) : "10";
				$opt['attachments_pagination_number'] = intval( $val );
				update_option(OPTION_PIE_REGISTER,$opt);
				$piereg_global_options = $opt;
				unset($opt);
			}
		...
		*/
		...
		function pie_admin_menu_style_enqueu(){
			wp_register_style( 'pie_menu_style_css', plugins_url("/assets/css/piereg_menu_style.css",__FILE__),false,'2.0', "all" );
		...
		function piereg_register_scripts(){
			wp_register_script('pie_prVariablesDeclaration_script',plugins_url("/assets/js/prVariablesDeclaration.js",__FILE__),false,false);
			wp_register_script('pie_prBackendVariablesDeclaration_script',plugins_url("/assets/js/prBackendVariablesDeclaration.js",__FILE__),false,false);
			wp_register_script('pie_prVariablesDeclaration_script_Footer',plugins_url("/assets/js/prVariablesDeclarationFooter.js",__FILE__),'','',true);
			wp_register_script('pie_prBackendVariablesDeclaration_script_Footer',plugins_url("/assets/js/prBackendVariablesDeclarationFooter.js",__FILE__),'','',true);
			wp_register_script('prBackendCustomLogo_script',plugins_url("/assets/js/prBackendCustomLogo.js",__FILE__),'','',true);
			wp_register_script('pie_datepicker_js',plugins_url("/assets/js/datepicker.js",__FILE__),array('jquery'),'2.0',false);
			wp_register_script('pie_drag_js',plugins_url("/assets/js/drag.js",__FILE__),'jquery','2.0',false);
			wp_register_script('pie_mCustomScrollbar_js',plugins_url("/assets/js/jquery.mCustomScrollbar.min.js",__FILE__),'jquery','2.0',false);			
			wp_register_script('pie_mousewheel_js',plugins_url("/assets/js/jquery.mousewheel.min.js",__FILE__),'jquery','2.0',false);
			wp_register_script('pie_sidr_js',plugins_url("/assets/js/jquery.sidr.min.js",__FILE__),'jquery','2.0',false);
			wp_register_script('pie_phpjs_js',plugins_url("/assets/js/phpjs.js",__FILE__),'jquery','2.0',false);
			wp_register_script('pie_registermain_js',plugins_url("/assets/js/pie-register-main.js",__FILE__),'jquery','2.0',false);
			wp_register_script('pie_alphanum_js',plugins_url("/assets/js/jquery.alphanum.js",__FILE__),array('jquery'),'2.0',false);
			wp_register_script('pie_validation_js',plugins_url("/assets/js/piereg_validation.js",__FILE__),'jquery','2.0',false);
		...
		function piereg_backendregister_scripts(){
			//Styles
			wp_register_style( 'pie_admin_css', plugins_url("/assets/css/admin.css",__FILE__),false,'1.0', "all" );
			wp_register_style( 'pie_restrict_widget_css', plugins_url("/restrict_widget/restrict_widget_css.css",__FILE__),false,'1.0', "all" );
			wp_register_style( 'pie_go_pro_menu_css', plugins_url("/assets/css/admin-go-pro-styles.css",__FILE__),false,'1.0', "all" );
			wp_register_style( 'pie_mCustomScrollbar_css', plugins_url("/assets/css/jquery.mCustomScrollbar.css",__FILE__),false,'2.0', "all" );
			//Scripts
			...
			$this->authenticate_license();	// PRO_Check			
		}
		...
		function print_multi_lang_script_vars(){
			...
			$this->pie_pr_dec_vars_array = array(
				...
				'ValidationMsgText14'					=> __("* You must fill one of the following fields","pie-register"),
				...
				'ValidationMsgText24'					=> __("* Invalid Phone No. Allowed Format (xxx) xxx-xxxx","pie-register"),
				'ValidationMsgText25'					=> __("* Minimum 10 Digits starting with Country Code without + sign.","pie-register"),
				...
				'ValidationMsgText39'					=> __("* This user is already taken","pie-register"),
				'ValidationMsgText40'					=> __("* Validating, please wait","pie-register"),
				'ValidationMsgText41'					=> __("* This username is available","pie-register"),
				'ValidationMsgText42'					=> __("* This user is already taken","pie-register"),
				'ValidationMsgText43'					=> __("* Validating, please wait","pie-register"),
				'ValidationMsgText44'					=> __("* This name is already taken","pie-register"),
				'ValidationMsgText45'					=> __("* This name is available","pie-register"),
				'ValidationMsgText46'					=> __("* Validating, please wait","pie-register"),
				'ValidationMsgText47'					=> __("* This name is already taken","pie-register"),
				'ValidationMsgText48'					=> __("* Please input HELLO","pie-register"),
				...
				'ValidationMsgText57'					=> __("* Alphabetic Letters only","pie-register"),
		...
		function print_multi_lang_backend_script_vars(){
			...
			foreach($fields_data as $k=>$field){
				...
				if( ($field['type'] == "honeypot" && !$this->piereg_pro_is_activate) || $field['type']=="submit" || $field['type']=="" || $field['type']=="form" || ($field['type']=="invitation" && $options["enable_invitation_codes"]=="0") ){
					continue;
				}
		...
		function pie_frontend_enqueu_scripts(){
			...
			if(isset($piereg_global_options['outputcss']) && $piereg_global_options['outputcss'] == 1){
				...
				if(isset($piereg_global_options['pr_theme']) && intval($piereg_global_options['pr_theme']) > 0 && $this->piereg_pro_is_activate){
					wp_register_style( 'pie_themes_css', plugins_url("/assets/css/theme/theme-".(intval($piereg_global_options['pr_theme'])).".css",__FILE__),false,'2.0', "all" );
		...
		function piereg_registration_form($attributes="") {
			...
			if(is_array($attributes) || $is_preview || $use_free_form ){
				...
				if( intval($id) != 0 )
				{
					...
					if($check_form_in_db == false || trim($check_form_in_db) == "")
					{
						...
						if( ($check_form_in_db['Status'] == "enable" && (!isset($check_form_in_db['IsDeleted']) || trim($check_form_in_db['IsDeleted']) != 1) ) || $is_preview){
							...
							return __("Currently this form is disabled by administrator.","pie-register");
						}
					}
				}else{
					$id = false;
				...
				if($id == false){
					return __("You are using wrong shortcode of Pie Register.","pie-register");
		...
		function add_action_links( $links, $file ) 
		{
			...
			if(!$this->piereg_pro_is_activate) $links[] = '<a style="color:#13ad11;" target="_blank" href="https://pieregister.com/">'.__("Go Pro","pie-register").'</a>';
			...
			if( ( $global_option_enabl && $page_restrtiction !== "0" ) || ($option_enabl) )
			{
				...
				if(trim($global_options['restrict_bot_content']) != "")
				{
					...
					*/
					...
					{
						if(strpos($_SERVER['HTTP_USER_AGENT'], trim($val)) !== false)
						{
							$validate_bot = strpos($_SERVER['HTTP_USER_AGENT'],trim($val));//check http user agent with defined
						}
		...
		*/
					...
					<div class="notice notice-info pie-admin-notice-3 is-dismissible">
						...
						<p>
							<a href="<?php echo $review_url; ?>" target="_blank" id="pie_review_in_start" class="button button-primary">
								<?php _e( 'Sure! I\'d love to give a review.', 'pie-register' ) ?>
							</a>	&nbsp;
							<a href="javascript:void(0);" class="button-secondary pie_review_in_link">
								<?php _e( 'No thanks', 'pie-register' ) ?>
							</a>
						</p>
					</div>
		...
		{
			$show_release_notice   = get_option( 'pie_promo_notice_for_free_1' );
			if ( empty( $show_release_notice ) ) {
				?>
                <div class="notice notice-info pie-admin-notice is-dismissible">
                	<p><?php echo sprintf( _e( 'Head up! Invitation Only Registration has been updated with new features. <a href="%s">Upgrade now.</a>','pie-register'),'https://pieregister.com/plan-and-pricing/'); ?></p>        	
                </div>
                <script type="text/javascript">
		...
		{
			update_option( 'pie_promo_notice_for_free_1', 'no' );
		}
		...
		function myplugin_meta_box_callback( $post ) { 
			...
			<?php $result = get_post_meta( $post->ID, '_piereg_post_restriction', true );?>
	            ...
	            <div class="piereg_restriction_field_area">
					...
					<div class="piereg_input">
						...
						<select id="piereg_post_visibility" name="post_restriction[piereg_post_visibility]">
			            	...
			            	<option value="before_login" <?php echo ($option == "before_login")? 'selected="selected"' : '' ?>><?php _e('Show to Non-Logged in Users',"pie-register") ?></option>
			                <?php
							global $wp_roles;
							$role = $wp_roles->roles;
							...
							{ 
								?>
								<option value="<?php echo strtolower(str_replace(" ","_",$value['name']));?>"<?php echo ($option == strtolower(str_replace(" ","_",$value['name']))) ? 'selected="selected"' : ''; ?>><?php _e("Show to","pie-register");echo " ".$value['name']; ?></option>
			                    <?php
							}
							?>
			            </select>
                ...
                <?php if( $this->piereg_pro_is_activate ) { ?>
                    ...
                    <h2><?php _e( 'Restrict For Search Engine(s)', 'pie-register' ); ?></h2>
                    <div class="piereg_restriction_field_area">
                        <div class="piereg_label">
                            <label><?php _e( 'Enable', 'pie-register' ); ?></label>
                        </div>
                        <div class="piereg_input">
                            ...
                            <div class="piereg_input_radio">
                                ...
                                <input type="radio" id="restriction_enabel_yes" class="piereg_bot_restriction_enabel" name="post_restriction[piereg_bot_restriction_enabel]" value="1" <?php echo ($option == 1)? 'checked="checked"' : '' ?> /><?php _e( 'Yes', 'pie-register' ); ?></label>
                            </div>
                            <div class="piereg_input_radio">
                                <label for="restriction_enabel_no">
                                <input type="radio" id="restriction_enabel_no" class="piereg_bot_restriction_enabel" name="post_restriction[piereg_bot_restriction_enabel]" value="0" <?php echo ($option == 0)? 'checked="checked"' : '' ?> /><?php _e( 'No', 'pie-register' ); ?></label>
                            </div>
                            <div class="piereg_input_radio">
                                <label for="restriction_enabel_global">
                                <input type="radio" id="restriction_enabel_global" class="piereg_bot_restriction_enabel" name="post_restriction[piereg_bot_restriction_enabel]" value="2" <?php echo ($option == 2)? 'checked="checked"' : '' ?> /><?php _e( 'Use Global', 'pie-register' ); ?></label>
                            </div>
                        </div>
                    </div>                    
                    <div class="piereg_restriction_field_area pieregister_bot_block_content_area">
                        <div class="piereg_label">
                            ...
                            <label for="piereg_bot_block_content"><?php _e( 'Bot Block Content', 'pie-register' ); ?></label>
                        </div>
                        <div class="piereg_input">
            ...
            <noscript><?php _e('Sorry, your browser does not support JavaScript!','pie-register'); ?></noscript>
		...
		*/
		...
		private function show_invitaion_code_user(){
			...
			<div style="width:100%">
				<h2><?php _e("Activation Code","pie-register");echo " : ".$inv_code; ?></h2>
		...
		function add_pie_form_button($context)
		{
			...
			printf('<a href="%s" class="thickbox button" id="add_pie_form" title="%s"><span style="%s" class="wp-media-buttons-icon"></span>%s</a>','#TB_inline?width=480&inlineId=select_pie_form',__("Add Pie Register Form", 'pie-register'),'background: url('.plugins_url('pie-register').'/assets/images/form-icon.png); background-repeat: no-repeat; background-position: left bottom;',__("Add Form","pie-register"));
		...
		function checkLoginPage()
		{
			...
			if($option['block_wp_login']==1 && $option['alternate_login'] > 0 && is_user_logged_in() && $current_page == $option['alternate_login'] && !$this->is_pr_preview )
			{	
				$this->afterLoginPage();
			}
		}
...
?>
					...
					if( $this->piereg_pro_is_activate ){
						if( isset($option['piereg_blk_ip']) && (isset($option['enable_blockedips']) && $option['enable_blockedips'] == 1 ) ) 
						{
							...
							if( $this->isUserIpsIsBlocked( ip2long($user_ip_address), $array_ips ) ) {
								$errors->add('login-error',apply_filters("piereg_ipaddress_blocked",__('Your IP address has been blocked by administrator.','pie-register')));
								$error_found++;
							}	
						}
						...
						if( isset($option['piereg_blk_username']) && (isset($option['enable_blockedusername']) && $option['enable_blockedusername'] == 1 ) )
						{
							...
							if(	$this->isUserNameIsBlocked($cred_userlogin,$array_username) ) {
								$errors->add('login-error',apply_filters("piereg_username_blocked",__('This user has been blocked by administrator.','pie-register')));
								$error_found++;
							}	
						}
						...
						{
							$cred_userlogin2	= $this->pie_post_array['log'];
							...
							if(	$this->isEmailAddressIsBlocked($cred_userlogin2,$array_emailaddr) ) {
								$errors->add('login-error',apply_filters("piereg_username_blocked",__('This email address has been blocked by administrator.','pie-register')));
								$error_found++;
							}	
						}
					}
					...
					if($option['captcha_in_login_value'] == 1 && !isset($this->pie_post_array['piereg_login_after_registration']) ){
						...
						if( $attempts ){
									...
									if( ($cookie_result1 == $cookie_result2) && ($cookie_result3 == $this->pie_post_array['piereg_math_captcha_login'])){
										...
										if($this->piereg_authentication($this->pie_post_array['log'],$this->pie_post_array['pwd'])){
											...
											if( $piereg_security_attempts_login_value ){
												$errors->add('login-error',apply_filters("blocked_ip_due_to_many_attempts_failed",__("We are sorry, but this IP address has been blocked due to too many recent failed login attempts.","pie-register")));
											}
										}
										$error_found++;
									}
							...
							elseif($option['capthca_in_login'] == 3 || (isset($this->pie_post_array["recaptcha_challenge_field"], $this->pie_post_array["recaptcha_response_field"]) && !empty($this->pie_post_array["recaptcha_challenge_field"]) && !empty($this->pie_post_array["recaptcha_response_field"]) ) ){
								...
								$captcha = "";
								if(isset($this->pie_post_array['g-recaptcha-response'])){
									$captcha=$this->pie_post_array['g-recaptcha-response'];
								}
					...
					if($error_found == 0){
						...
						else
						{
							...
							if($this->piereg_authentication($this->pie_post_array['log'],$this->pie_post_array['pwd'])){
								...
								if( $piereg_security_attempts_login_value ){
									$user = new WP_Error('piereg_authentication_failed', __("We are sorry, but this IP address has been blocked due to too many recent failed login attempts.","pie-register"));
								}
							}
						}
						...
						if ( is_wp_error($user))
						{
							...
							{
								$user_login_error = apply_filters('pie_invalid_username_password_msg_txt','<strong>'.ucwords(__("error","pie-register")).'</strong>: '.__("Invalid username or password","pie-register").'. <a href="'.$this->pie_lostpassword_url().'" title="'.__("Password Lost and Found","pie-register").'">'.__("Lost your password?","pie-register").'</a>');
							}else if(strpos(strip_tags($user_login_error),'password you entered',9) > 10)
							{
								$user_login_error = apply_filters('pie_invalid_user_password_msg_txt','<strong>'.ucwords(__("error","pie-register")).'</strong>: '.__("Invalid username or password","pie-register").' <strong>'.$this->pie_post_array['log'].'</strong> '.__("is incorrect","pie-register").'. <a href="'.$this->pie_lostpassword_url().'" title="'.__("Password Lost and Found","pie-register").'">'.__("Lost your password?","pie-register").'</a>');
							}
			...
			if( ($this->check_enable_payment_method()) == "true" && $is_active == 1)
			{
				...
				if(isset($piereg_form_pricing_fields["form_id_".(intval($user_registered_form_id))]) && !empty($piereg_form_pricing_fields["form_id_".(intval($user_registered_form_id))]) && empty($piereg_use_starting_period)){
					...
					if(date_i18n('Y-m-d H:i:s', strtotime('+'.$pricing_for.' '.$pricing_for_period, strtotime($user->data->user_registered))) < date_i18n('Y-m-d H:i:s')){
						...
						$errors->add('login-error',__("your account has been suspended, Please renew your account","pie-register"));
				...
				if( ($payment_activation_cycle) != 0)
				{
					...
					{
						$errors->add('login-error',__("Your account has been blocked","pie-register"));
						update_user_meta( $user->ID, 'active', 0);
						if((isset($pie_reg['user_enable_user_temp_blocked_notice']) && $pie_reg['user_enable_user_temp_blocked_notice'] == 1) ) 
							$this->wp_mail_send($user->data->user_email,"user_temp_blocked_notice");
						if((isset($pie_reg['user_enable_user_renew_temp_blocked_account_notice']) && $pie_reg['user_enable_user_renew_temp_blocked_account_notice'] == 1) )
		...
		function AddPanel()
		{
			...
			$pie_page_suffix_14 = add_submenu_page( 'pie-register', 'Notifications Setting', __('Notifications',"pie-register") , 'manage_options', 'pie-notifications', array($this, 'PieRegNotifications') );
			...
			if(	is_plugin_active('pie-register-aweber/pie-register-aweber.php') || is_plugin_active('pie-register-mailchimp/pie-register-mailchimp.php') )
			{
				...
				$pie_page_suffix_16 = add_submenu_page( 'pie-register', 'Export Users', __('Export Users',"pie-register") , 'manage_options', 'pie-export-users', array($this, 'PieRegExportUsers'));
			...
			// Restrict Users by username and ip address
			$pie_page_suffix_15 = add_submenu_page( 'pie-register', 'Block Users', __('Block Users',"pie-register") , 'manage_options', 'pie-black-listed-users', array($this, 'PieRegRestrictUsers'));
			...
			$pie_page_suffix_11 = add_users_page( 'Unverified Users', __('Unverified Users',"pie-register") , 'manage_options', 'unverified-users', [$this, 'Unverified'] );
			...
			if(isset($_POST['piereg_reg_form_nonce']) && wp_verify_nonce( $_POST['piereg_reg_form_nonce'], 'piereg_wp_reg_form_nonce' ))
			{
				...
				foreach($this->pie_post_array['field'] as $k=>$fv){
					...
					if($fv['type'] == 'password'){
						...
						if(isset($fv['field_rule_operator']) && ($fv['field_rule_operator'] == '&lt;' || $fv['field_rule_operator'] == '&gt;') ) {
							$fv['field_rule_operator']	= html_entity_decode($fv['field_rule_operator']);
						}
				...
				if(isset($_POST['form_id']) and intval(base64_decode($_POST['form_id'])) != 0 and isset($_POST['page']) and $_POST['page'] == "edit")
				{
					...
					update_option("piereg_form_fields_".$fields_id,serialize($this->pie_post_array['field']));
					...
					update_option(OPTION_PIE_REGISTER,$options);
					$_field['Id'] 		= $fields_id;
					$_field['Title'] 	= $this->pie_post_array['field']['form']['label'];
					$_field['Views'] 	= "0";
					$_field['Entries'] 	= "0";
		...
		function check_register_form()
		{
			...
			else if(($this->check_enable_payment_method()) == "true")
			{
				...
				)
				{
					...
					{
						$this->pie_post_array['error'] = $_POST['error_stripe'];
					}
					    ...
					    $this->pie_post_array['error'] = $_POST['error_authorizeNet'];
                    }
                    else {
						$this->pie_save_registration();
					}
				...
				{
					$this->pie_post_array['error'] = __("Please select any payment method","pie-register");	
				}
				else{
					$this->pie_save_registration();	
				}
		...
		function pie_save_registration()
		{
			...
			if(sizeof($errors->errors) == 0)
			{
				...
				$user_data = array(
								   ...
								   'user_email' => strtolower($_POST['e_mail']),
								   'role' 		=> get_option('default_role')
								  );
				
				if(isset($_POST['first_name']) && !empty($_POST['first_name']) )
				{
					$display_name = esc_html($_POST['first_name']).((isset($_POST['last_name']) && !empty($_POST['last_name']))?" ".esc_html($_POST['last_name']):"");
				...
				*/
				...
				if( $isPayPalStandard == false && $this->check_enable_payment_method() == "true" && ($checkStartPayment || isset($_POST['select_payment_method'])  ) )
				{
					...
					if( $_POST['select_payment_method'] == "stripe" || $_POST['select_payment_method'] == "authorizeNet"){
						...
						*/
						...
						/*echo "<pre>";
						print_r($user);
						print_r("check_payment_method_".$_POST['select_payment_method']); 
						echo "</pre>";
						//die;
						*/
						...
						if(!empty($_POST['error'])) 	{
							...
							$this->pie_post_array['error'] 	 		= $_POST['error'];
						}
						
						// If payment success
						if(!empty($_POST['success']))	$this->pie_post_array['success'] = $_POST['success'];				
				...
				else if($option_user_verification == 1 )//Admin Verification
				{
					...
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration_admin_veri",__("Thank you for your registration. You will be notified once the admin approves your account.",'pie-register'));
				}
				...
				else if($option_user_verification == 2 )//E-Mail Link Verification
				{
					...
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration_email_veri",__("Thank you for your registration. An activation link has been sent to verify your email address.",'pie-register'));
				...
				else if($option_user_verification == 3 )//Admin & E-Mail Verification
				{
					...
					if((isset($option['user_enable_admin_verification']) && $option['user_enable_admin_verification'] == 1) && !wp_mail($_POST['e_mail'], $subject, $message , $headers)){
						$this->pr_error_log("'The e-mail could not be sent. Possible reason: mail() function may have disabled by your host.'".($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
					}
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration_admin_email_veri",__("Thank you for your registration. You will be notified once the admin approves your account",'pie-register'));
				}
		...
		function Unverified(){
			...
			<form method="post" id="pie-unverified-users">
				<input type="hidden" name="page" value="pie_un_users_list">
				<?php
		...
		function verifyUsers()
		{
			...
			if($valid)
			{
				...
				$this->pie_post_array['notice'] = __("User(s) has been activated");
			}
			else
				$this->pie_post_array['notice'] = "<strong>".__('error','pie-register').":</strong>".__("Please select a user to send emails to", "pie-register");
		}
		function PaymentLink()
		{
			...
			if( is_array($valid)) 
			{
				...
				if($sent > 0)
					$this->pie_post_array['notice'] = __("Payment Link Emails have been re-sent", "pie-register");
				else
		...
		function AdminDeleteUnvalidated()
		{
			...
			{	
				...
				$this->pie_post_array['notice'] = __("User(s) has been deleted");
			}
		}
		...
		function PieRegSettingsProcess() {
			...
			if(isset($_POST['piereg_settings_allusers']) && wp_verify_nonce( $_POST['piereg_settings_allusers'], 'piereg_wp_settings_allusers' )) {
				...
				if($this->piereg_pro_is_activate){
					$update['login_after_register'] = isset($this->pie_post_array['login_after_register']) ? intval($this->pie_post_array['login_after_register']) :0;
				}
				...
				if( isset($this->pie_post_array['is_advanced']) && $this->pie_post_array['is_advanced'] == 1 )
				{
					$update['pr_theme']				= intval($this->pie_post_array['pr_theme']);	
				} 
				else 
				{
				...
				if($this->piereg_pro_is_activate){
					$update['piereg_security_attempts_login_value'] = isset($this->pie_post_array['piereg_security_attempts_login_value']) ? intval($this->pie_post_array['piereg_security_attempts_login_value']) : "";
				}
				$update['security_attempts_login'] = isset($this->pie_post_array['security_attempts_login']) ? intval($this->pie_post_array['security_attempts_login']) : 0; 
				$update['security_attempts_login_time'] = isset($this->pie_post_array['security_attempts_login_time']) ? intval($this->pie_post_array['security_attempts_login_time']) : 0;
				...
				if($this->piereg_pro_is_activate){
					$update['piereg_security_attempts_forgot_value'] = isset($this->pie_post_array['piereg_security_attempts_forgot_value']) ? intval($this->pie_post_array['piereg_security_attempts_forgot_value']) : "";
				}
				$update['security_attempts_forgot_time'] = isset($this->pie_post_array['security_attempts_forgot_time']) ? intval($this->pie_post_array['security_attempts_forgot_time']) : 0;
				$update['security_attempts_forgot'] = isset($this->pie_post_array['security_attempts_forgot']) ? intval($this->pie_post_array['security_attempts_forgot']) : 0;
		...
		function PieRegRedirectSettingsAction(){
			...
			/*	Add Record	*/
			elseif(isset($this->pie_post_array['redirect_settings_add_new']))
			{
				$this->piereg_get_wp_plugable_file(true); // require_once pluggable.php
				...
				{
					if( empty($this->pie_post_array['logged_in_url']) && empty($this->pie_post_array['log_out_url']) && $this->pie_post_array['log_in_page'] == "-1" && $this->pie_post_array['log_out_page'] == "-1" ) 
					{
						$this->pie_post_array['error'] = __("Please select any page or url for user role.","pie-register");	
					}
					elseif(
						(isset($this->pie_post_array['piereg_user_role']) && !empty($this->pie_post_array['piereg_user_role'])) &&
						(isset($this->pie_post_array['logged_in_url']) && isset($this->pie_post_array['log_out_url'])) &&
						(!empty($this->pie_post_array['logged_in_url']) || !empty($this->pie_post_array['log_out_url']) || !empty($this->pie_post_array['log_in_page']) || !empty($this->pie_post_array['log_out_page']))
					  ){
						
						$insr_piereg_user_role 	= trim($this->pie_post_array['piereg_user_role']);
						$insr_logged_in_url 	= urlencode(trim($this->pie_post_array['logged_in_url']));
						$insr_log_in_page 		= intval($this->pie_post_array['log_in_page']);
						$insr_log_out_url 		= urlencode(trim($this->pie_post_array['log_out_url']));
						$insr_log_out_page 		= intval($this->pie_post_array['log_out_page']);
						$insr_status 			= "1";
						...
						if($wpdb->query( $wpdb->prepare($sql, $insr_piereg_user_role, $insr_logged_in_url, $insr_log_in_page, $insr_log_out_url, $insr_log_out_page, $insr_status) ))
							$this->pie_post_array['notice'] = __("Successfully add new record","pie-register");
						else
							$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
					}else{
						$this->pie_post_array['error'] = __("Invalid field(s)","pie-register");
					}
				}else{
					$this->pie_post_array['error'] = __("Wrong nonce","pie-register");
				}
			...
			/*	Update Record	*/
			elseif(isset($this->pie_post_array['redirect_settings_update'])){
				
				if(isset($this->pie_post_array['id']) && $this->pie_post_array['id'] && isset($this->pie_post_array['logged_in_url']) && isset($this->pie_post_array['log_out_url'])){
					
					$upd_logged_in_url 	= urlencode(trim($this->pie_post_array['logged_in_url']));
					$upd_log_in_page 	= intval($this->pie_post_array['log_in_page']);
					$upd_log_out_url 	= urlencode(trim($this->pie_post_array['log_out_url']));
					$upd_log_out_page 	= intval($this->pie_post_array['log_out_page']);
					$upd_id 			= intval($this->pie_post_array['id']);
					...
					if( $wpdb->query( $wpdb->prepare($sql, $upd_logged_in_url, $upd_log_in_page, $upd_log_out_url, $upd_log_out_page, $upd_id) ) )
						$this->pie_post_array['notice'] = __("Successfully update record","pie-register");
					else
						$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
				}else{
					$this->pie_post_array['error'] = __("Invalid field(s)","pie-register");
		...
		function PieRegInvitationCodes()
		{
			...
			if( isset($_POST['piereg_invitation_code_nonce']) && wp_verify_nonce( $_POST['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ) )
			{
				...
				if( isset($this->pie_post_array['submit_invite_email']) &&  $this->piereg_pro_is_activate ){
					
					/**
					 * Saving Invitation Code Email Template.
					 */
					$piereg['pie_email_linkpage'] 	= isset($this->pie_post_array['pie_email_linkpage']) ? intval($this->pie_post_array['pie_email_linkpage']) : 0;
					$piereg['pie_email_invitecode'] = isset($this->pie_post_array['pie_email_invitecode']) ? $this->pie_post_array['pie_email_invitecode'] : '';
					$piereg['pie_name_from'] 		= isset($this->pie_post_array['pie_name_from']) ? $this->pie_post_array['pie_name_from'] : '';
					$piereg['pie_email_from'] 		= isset($this->pie_post_array['pie_email_from']) ? $this->pie_post_array['pie_email_from'] : '';
					$piereg['pie_email_subject'] 	= isset($this->pie_post_array['pie_email_subject']) ? $this->pie_post_array['pie_email_subject'] : '';
					$piereg['pie_email_content'] 	= isset($this->pie_post_array['pie_email_content']) ? $this->pie_post_array['pie_email_content'] : '';
					$_data_updated	= true;
				}
				...
				if( isset($this->pie_post_array['invi_del_id']) ) 
				{
					...
					if(isset($this->pie_post_array['piereg_invitation_code_nonce']) && wp_verify_nonce( $this->pie_post_array['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ))
					{
						if($wpdb->query( $wpdb->prepare("DELETE FROM ".$codetable." WHERE id = %s", $this->pie_post_array['invi_del_id']) ))	
							$this->pie_post_array['notice'] = __("The Invitation Code has been deleted","pie-register");
						else
							$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
					}else{
						$this->pie_post_array['error'] = __("Wrong nonce","pie-register");
				...
				else if( isset($this->pie_post_array['send_invite_email']) ) 
				{
					$this->piereg_get_wp_plugable_file(true); // require_once pluggable.php
					if(isset($this->pie_post_array['piereg_invitation_code_nonce']) && wp_verify_nonce( $this->pie_post_array['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ))
					{
						...
						foreach($to_emails as $email){
							...
							$pageURL 	= add_query_arg(array('action'=>'ic','code'=>$piereg['pie_email_invitecode'],'key'=>urlencode($key)),get_page_link($piereg['pie_email_linkpage']));
							$emailBody 	= str_replace('%invitation_link%',$pageURL,$piereg['pie_email_content']);
							
							if(wp_mail($email, $subject, $emailBody, $headers)){
								update_option('pie_invitation_link_'.$key,$piereg['pie_email_invitecode']);
								$this->pie_post_array['notice'] = __("Email has been sent successfully.","pie-register");								
							} else {
								$this->pr_error_log("'The e-mail could not be sent. Possible reason: mail() function may have disabled by your host.'".($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));								
							}
						}
				...
				else if( isset($this->pie_post_array['piereg_codepass']) || isset($this->pie_post_array['invitation_code_prefix']) ) 
				{
						...
						if(isset($this->pie_post_array['add_code'])){
							if(!$this->piereg_pro_is_activate){
								if(isset($this->pie_post_array['invitation_code_prefix'])){
									$this->pie_post_array['error'] = __("Please Activate Pro Version.","pie-register");
								}
							}
							if($this->piereg_pro_is_activate){
								if(empty($this->pie_post_array['invitation_code_prefix']) && !empty($this->pie_post_array['invitation_code_numbers'])){
									$this->pie_post_array['error'] = __("Code Prefix Should not be empty.","pie-register");
								}
								if(empty($this->pie_post_array['invitation_code_prefix']) && empty($this->pie_post_array['invitation_code_numbers'])){
									if(empty($this->pie_post_array['piereg_codepass']) || trim($this->pie_post_array['piereg_codepass']) == ''){
										$this->pie_post_array['error'] = __("Code field should not be empty.","pie-register");
									}
								}
							} else {
								if(empty($this->pie_post_array['piereg_codepass']) || trim($this->pie_post_array['piereg_codepass']) == ''){
							...
							if(isset($this->pie_post_array['invitation_code_usage']) && !is_numeric($this->pie_post_array['invitation_code_usage']) && trim($this->pie_post_array['invitation_code_usage']) != ''){
								...
								$this->pie_post_array['error'] .= __("Usage only allows numeric characters.","pie-register");
							}elseif(isset($this->pie_post_array['invitation_code_usage']) && $this->pie_post_array['invitation_code_usage'] < 0){
								if(empty($this->pie_post_array['piereg_codepass']) || trim($this->pie_post_array['piereg_codepass']) == ''){
									$this->pie_post_array['error'] .= '<br/>';
								}
								$this->pie_post_array['error'] .= __("Usage should not be less than 0 (zero).","pie-register");
							}
							...
							if($update['codepass'] != ""){
								...
								if($this->piereg_pro_is_activate){
									$invite_length = $this->pie_post_array['invitation_code_numbers'];
									$invite_prefix = $this->pie_post_array['invitation_code_prefix'];
									$codespasses = [];
									if($invite_prefix != ''){
										for($i = 1; $i <= $invite_length; $i++){
											$invite_random = '0123456789abcdefghijklmnopqrstuvwxyz';
											$invite_code = $invite_prefix . '_' . substr(str_shuffle($invite_random), 0, 4);
											array_push($codespasses,$invite_code);
										}
									}
								}
							}
							...
							if(!$codeadded && $count_code != $count_added_code && !isset($this->pie_post_array['error'])){
								...
								if(isset($this->pie_post_array['invitation_expiry_date']) && $this->piereg_pro_is_activate)
								{
									$piereg["invitation_expiry_date"] = $this->pie_post_array['invitation_expiry_date'];
									if(update_option(OPTION_PIE_REGISTER,$piereg))
									{
										$this->pie_post_array['notice'] = __("Invitation Code(s) added successfully","pie-register");
									}
		...
		function InsertCode($name)
		{
				...
				if($this->piereg_pro_is_activate){
					$expiry_date 	= date("Y-m-d", strtotime($_POST['invitation_expiry_date']));
					$today 			= date('Y-m-d');
					$expiry_date 	= (strtotime($expiry_date) < strtotime($today)) ? '' : $expiry_date; 
				} else {
		...
		{	
			...
			if(isset($_POST['piereg_export_users_nonce']) && wp_verify_nonce( $_POST['piereg_export_users_nonce'], 'piereg_wp_exportusers_nonce' ))
			{
				...
				$fields 	= "";
				if(sizeof($this->pie_post_array['pie_fields_csv']) > 0) {
		...
		function importUsers()
		{
			if(isset($_POST['piereg_import_users_nonce']) && wp_verify_nonce( $_POST['piereg_import_users_nonce'], 'piereg_wp_importusers_nonce' ))
			{
				...
				if($already_exist){
					if(isset($this->pie_post_array['update_existing_users']) && $this->pie_post_array['update_existing_users'] == "yes"){
						$this->pie_post_array['success_message'] .= "<br />".__("$already_exist user(s) Update.",'pie-register');
					}else{
			...
			if( isset($_POST['action']) && $_POST['action'] == 'pie_reg_update' )
			{
				if(isset($_POST['payment_gateway_page'],$_POST['piereg_payment_gateway_page_nonce']) && wp_verify_nonce($_POST['piereg_payment_gateway_page_nonce'], 'piereg_wp_payment_gateway_page_nonce') )
				{
					...
					if(isset($_POST['piereg_admin_email_notification']) && wp_verify_nonce( $_POST['piereg_admin_email_notification'], 'piereg_wp_admin_email_notification' ))
					{
						...
						$update['enable_admin_notifications']	= intval($this->pie_post_array['enable_admin_notifications']);
				...
				{
					$this->pie_post_array	= $this->piereg_sanitize_post_data( 'piereg_restrict_users',  ( (isset($_POST) && !empty($_POST))?$_POST : array() ) );
					
					$this->piereg_get_wp_plugable_file(true); // require_once pluggable.php
					if( isset( $this->pie_post_array['restrict_user_by_ip'] ) )
					{
						$update['enable_blockedips'] 		= isset($this->pie_post_array['enable_blockedips']) ? intval($this->pie_post_array['enable_blockedips']) : "";
						$update['piereg_blk_ip'] 			= $this->pie_post_array['piereg_blk_ip'];
						
					} 
					else if( isset( $this->pie_post_array['restrict_user_by_username'] ) )
					{
						$update['enable_blockedusername'] 	= isset($this->pie_post_array['enable_blockedusername']) ? intval($this->pie_post_array['enable_blockedusername']) : ""; 
						$update['piereg_blk_username'] 		= $this->disable_magic_quotes_gpc($this->pie_post_array['piereg_blk_username']);
					}
					else if( isset( $this->pie_post_array['restrict_user_by_email'] ) )
					{
						$update['enable_blockedemail'] 	=  isset($this->pie_post_array['enable_blockedemail']) ? intval($this->pie_post_array['enable_blockedemail']) : ""; 
						$update['piereg_blk_email'] 	= $this->disable_magic_quotes_gpc($this->pie_post_array['piereg_blk_email']);
					}
					else
					{
						$this->pie_post_array['error'] = __("Wrong nonce","pie-register");
		...
		*/
		...
		function addHoneyPotField($field,$no,$field_type)
		{
			...
			echo '<img src="'.plugins_url('/assets/images/honeypot.png', __FILE__).'" align="left" /><input disabled="disabled" id="'.$id.'" name="'.$name.'" class="input_fields"  placeholder="'.((isset($field['placeholder']))?$field['placeholder']:"").'" type="text" value="'.((isset($field['default_value']))?$field['default_value']: "" ).'" data-field-post-name="'.$fieldPostName.'" />';
		...
		function addDate($field,$no)
		{
			...
			echo	'<div class="time date_format_field" id="datepicker_'.$no.'" '.$datepicker.'>
					  <input disabled="disabled" type="text" class="input_fields">
	  				  <img src="'.plugins_url('pie-register').'/assets/images/calendar.png" id="calendar_image_'.$no.'" '.$calendar_icon.' /> </div>';
		...
		{
			echo '<img src="'.plugins_url('pie-register').'/assets/images/pagebreak.png" style="max-width:100%;" />';
		}
		...
		function addCaptcha($field,$no)
		{
			...
			if(isset($field['recaptcha_type']) && $field['recaptcha_type'] == 2)
				echo '<img id="captcha_img" src="'.plugins_url('pie-register').'/assets/images/new-recatpcha.png" data-captcha-img-src="'.plugins_url('pie-register').'/assets/images/" />';
			else
				echo '<img id="captcha_img" src="'.plugins_url('pie-register').'/assets/images/recatpcha.jpg" data-captcha-img-src="'.plugins_url('pie-register').'/assets/images/" />';
			
			echo '<p>'.'<strong>'.__("Note","pie-register").':</strong> '.__("Please make sure that Re-captcha keys are entered in Settings page","pie-register").'</p>';
			echo '<input type="hidden" id="default_'.$field['type'].'">';	
		}
		function addMath_Captcha($field,$no)
		{
			...
			echo '<img id="captcha_img" src="'.plugins_url('pie-register').'/assets/images/math_catpcha.png" />';	
			echo '<input type="hidden" id="default_'.$field['type'].'">';
		}
		function addList($field,$no)
		{
			...
			echo '<img src="'.plugins_url('pie-register').'/assets/images/plus.png" />';
		}
		...
		function deleteUsers($user_id = 0,$user_email = "",$user_registered = "",$autodelete = false)
		{
			...
			if( ($grace != 0 and $user_id != 0) and ($user_email != "" and $user_registered != "") and ($register_type != '' and $register_type != "admin_verify") && $is_active	== 0 )
			{
				...
				if($user_registered < $date)
				{
					...
					if( $autodelete === false )
					{
						$errors->add('login-error',apply_filters("piereg_msg_deleted_unverified_user",__('Your account has been removed due to not verifying it within the grace period.','pie-register')));
						wp_logout();
					}
		...
		function showForgotPasswordForm()
		{
			...
			$option 		= get_option(OPTION_PIE_REGISTER);
			if(!is_admin() && is_user_logged_in() && $option['redirect_user']==1 && !$this->is_pr_preview)
			{
				if(did_action('send_headers'))  $this->afterLoginPage();
				else return "";
			}
			else
			{
		...
		function get_paypalstandard($value,	$payment_gateways=array())
		{
			...
			{
				$data .= '<img src="'.plugins_url('pie-register/assets/images/paypal_std_btn.png').'">';
		...
		function Add_payment_option($send_data,$paymeny_gatways) // Only For Paypal
		{
			...
			if($check_payment["enable_paypal"] == 1 && !(empty($check_payment['paypal_butt_id'])) )
			{
				...
				{
					$send_data .= '<option data-img="'.plugins_url('pie-register/assets/images/paypal_std_btn.png').'" value="PaypalStandard">'.(__("Paypal (Standard)","pie-register")).'</option>';
				}
			}
		...
		{ 
			...
			if( isset($_POST['piereg_renew_account_nonce']) && wp_verify_nonce( $_POST['piereg_renew_account_nonce'], 'piereg_wp_renew_account_nonce' ) )
			{
				...
				if( isset($_POST['select_payment_method']) and trim($_POST['select_payment_method']) != "" )
				{
					...
					$this->pie_post_array['error'] = apply_filters("piereg_Please_Select_any_payment_method",__("Please Select any payment method","pie-register"));
				}
			} else {
				$this->pie_post_array['error'] = 'Wrong nonce';
			}
		}
		...
		function wp_mail_send($to_email = "",$key = "",$additional_msg = "",$msg = "",$email_variable = array())
		{
			...
			if(trim($to_email) != "" and trim($key) != "" )
			{
				...
				if(!wp_mail($to,$subject,$message,$headers))
				{
					$errors->add('check-error',apply_filters("piereg_problem_and_the_email_was_probably_not_sent",__("There was a problem and the email was probably not sent.",'pie-register')));
				}
				else{
					if(trim($msg) != "")
					{
		...
		function pireg_update_invitation_code_cb_url()
		{
			...
			if( isset($_POST['data']))
			{
				if(trim($_POST['data']['id']) != "" and trim($_POST['data']['value']) != "" and trim($_POST['data']['type']) != "")
				{
					...
					else if(trim($_POST['data']['type']) == "expiry_date" && $this->piereg_pro_is_activate)
					{
						if($_POST['data']['value'] != ''){
							$expiry_date = date("Y-m-d", strtotime($_POST['data']['value']));
							$today = date('Y-m-d');
							$expiry_date = (strtotime($expiry_date) < strtotime($today)) ? '' : $expiry_date; 
							$sql = "UPDATE `{$codetable}` SET `expiry_date`=%s WHERE `id` = ".((int)$_POST['data']['id'])."";
						}
					}
			...
			if(isset($_POST['import_export_settings']) && $_POST['import_export_settings']){
				...
				elseif( isset($_POST['import_general_settings']) && isset($_FILES['import_general_settings_file']) && $_FILES['import_general_settings_file']['name'] != "" ){
					...
					if(pathinfo($_FILES['import_general_settings_file']['name'],PATHINFO_EXTENSION) == "json"){
						...
						$this->pie_post_array['error'] = __("Invalid File","pie-register");
					}
				}
				...
				elseif( isset($_POST['import_invitations_codes']) && isset($_FILES['import_invitations_codes_file']) && $_FILES['import_invitations_codes_file']['name'] != "" ){
					...
					}elseif(pathinfo($_FILES['import_invitations_codes_file']['name'],PATHINFO_EXTENSION) == "csv"){
						$this->import_invitation_codes(true);
					}else{
						$this->pie_post_array['error'] = __("Invalid File","pie-register");
					}
				}
			...
			if(isset($_POST['piereg_import_general_settings']) && wp_verify_nonce( $_POST['piereg_import_general_settings'], 'piereg_wp_import_general_settings' ))
			{
				...
				if(isset($array_json_file['piereg_plugin_db_version']) && !empty($array_json_file['piereg_plugin_db_version'])){
					...
					if($pie_plugin_db_version != $import_file_db_version){
						$this->pie_post_array['error'] = __("The file version is not compatible with this version","pie-register");
						return false;
					}
				}else{
					$this->pie_post_array['error'] = __("The file version is not compatible with this version","pie-register");
						...
						case "pie_us_states":
							if($arr_val){
								...
								if(is_array($result_merge_array)){
									...
									$this->pie_post_array['success_message'] = __("Successfully import all settings","pie-register");
								}
								unset($db_option);
		...
		*/
		function export_invitation_codes($is_download = true){
			if(isset($_POST['piereg_export_invitations_codes']) && wp_verify_nonce( $_POST['piereg_export_invitations_codes'], 'piereg_wp_export_invitations_codes' ))
			{
				...
				$sql = "SELECT `id`, `created`, `modified`, `name`, `count`, `status`, `code_usage`, `expiry_date` FROM {$invitation_code_table_name} ORDER BY `id` ASC";
				$result_invitaion_code = $wpdb->get_results( $sql ); #WPS
				if(isset($wpdb->last_error) && !empty($wpdb->last_error)){
					$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
				}
				...
				if(!empty($result_invitaion_code)){
					...
					$this->pie_post_array['error'] = __("Record(s) not found","pie-register");
				}
			}else{
				$this->pie_post_array['error_message'] = __("Wrong nonce","pie-register");
			}
		...
		/*
			*	Import Invitaion Code
		*/
		function import_invitation_codes($_is_csv=false){
			...
			{
				$array_json_file = array();
				if($_is_csv)
				{
					$csv_data = $this->csv_to_array($_FILES['import_invitations_codes_file']['tmp_name'],',');
					$array_json_file['invitation_codes'] = $csv_data;
				}
				else{
				...
				if(isset($array_json_file['invitation_codes']) && is_array($array_json_file['invitation_codes'])){
					global $wpdb;
					$invitation_code_table_name = $wpdb->prefix."pieregister_code";
					$invitation_code_sql = "CREATE TABLE IF NOT EXISTS ".$invitation_code_table_name."(`id` INT( 5 ) NOT NULL AUTO_INCREMENT PRIMARY KEY ,`created` DATE NOT NULL ,`modified` DATE NOT NULL ,`name` TEXT NOT NULL ,`count` INT( 5 ) NOT NULL ,`status` INT( 2 ) NOT NULL ,`code_usage` INT( 5 ) NOT NULL, `expiry_date` DATE NOT NULL) ENGINE = MYISAM ;";
					...
					$insert_sql = "INSERT INTO `".$invitation_code_table_name."` (`id`, `created`, `modified`, `name`, `count`, `status`, `expiry_date`, `code_usage`) VALUES ";
					
					if($_is_csv)
					{
						$array_json_file['invitation_codes'] = array_map(function($code) {
							return array(
								'name' => $code['Code'],
								'code_usage' => $code['Usage']
							);
						}, $array_json_file['invitation_codes']);
					}
					...
					foreach($array_json_file['invitation_codes'] as $inv_key=>$inv_val)
					{
						...
						if(isset($inv_val['name']) && $inv_val['name'] != ""){
							...
							$query_values .= "'".((isset($inv_val['status']) && $inv_val['status'] != "")?$inv_val['status']: 1)."',";
							$query_values .= "'".((isset($inv_val['expiry_date']) && $inv_val['expiry_date'] != "")?$inv_val['expiry_date']: "0000-00-00")."',";
							$query_values .= "'".((isset($inv_val['code_usage']) && $inv_val['code_usage'] != "")?$inv_val['code_usage']: 1)."'),";
						}
						if($count_query == 100){
							$count_query = 0;
							$query_values = rtrim($query_values,",");
					...
					$this->pie_post_array['success_message'] = __("Successfully import invitation codes","pie-register");					
				...
				if(isset($array_json_file['admin_email_template'],$array_json_file['user_email_template'])){
					if(is_array($array_json_file['admin_email_template']) && is_array($array_json_file['user_email_template'])){
						...
						if(is_array($merge_email_templates)){
							...
							$this->pie_post_array['success_message'] = __("Successfully import e-mail templates","pie-register");
						}
					}
				}
			}else{
		...
		*/
		function import_all_users_data_with_custom_fields(){
			if(isset($_POST['piereg_import_user_custom_data']) && wp_verify_nonce( $_POST['piereg_import_all_user_custom_data'], 'piereg_wp_import_all_user_custom_data' )){
				...
				if( $this->check_json_file_db_version($all_users_data) ){
					...
					$this->pie_post_array['successfull_import_all_users_data'] 		= 0;
					$this->pie_post_array['unsuccessfull_import_all_users_data'] 	= 0;
		...
		function piereg_plugin_row_meta( $links, $file ) {
			if ( $file == PIEREG_PLUGIN_BASENAME ) {
				$row_meta = array(
					'documentation'		=>	'<a href="' . esc_url( apply_filters( 'pieregister_docs_url', 'https://pieregister.com/manual/introduction/' ) ) . '" title="' . esc_attr( __( 'View Pie Register Documentation', 'pie-register' ) ) . '" target="_blank">' . __( 'Documentation', 'pie-register' ) . '</a>',
					'support'	=>	'<a href="' . esc_url( apply_filters( 'pieregister_support_url', 'http://pieregister.com/forum/' ) ) . '" title="' . esc_attr( __( 'Visit Customer Support Forum', 'pie-register' ) ) . '" target="_blank">' . __( 'Support', 'pie-register' ) . '</a>',
				);				
			...
			if( isset($_POST['piereg_activate_license_key'], $_POST['save_license_key_settings']) )
			{
				if(isset($_POST['piereg_license_key'], $_POST['piereg_license_email'], $_POST['piereg_license_key_nonce'] ) && !empty($_POST['piereg_license_key']) && !empty($_POST['piereg_license_email']) )
				{
					if(wp_verify_nonce( $_POST['piereg_license_key_nonce'], 'piereg_wp_license_key_nonce' )){
						...
						$this->pie_post_array	= $this->piereg_sanitize_post_data( 'piereg_license_key',  ( (isset($_POST) && !empty($_POST))?$_POST : array() ) );
						$piereg_api_manager_menu = new Piereg_API_Manager_Example_MENU();
						$piereg_api_manager_menu->validate_options( array("api_key"=> trim($this->pie_post_array['piereg_license_key']), "activation_email"=> trim($this->pie_post_array['piereg_license_email']) ) );
					}
					else{
						$this->pie_post_array['error'] = __("Wrong nonce","pie-register");
					}
			...
			if( isset($_POST['piereg_activate_addon_license_key'], $_POST['save_addon_license_key_settings']) )
			{
				if(isset($_POST['piereg_addon_license_key_nonce']))
				{
					if(wp_verify_nonce( $_POST['piereg_addon_license_key_nonce'], 'piereg_wp_addon_license_key_nonce' )){
						...
						{
							$LK_options['api_key']			= trim($_POST['piereg_addon_license_key']);
				...
				if( ($_GET['action'] == "current_email_verify" ) && (isset($_GET['key']) && !empty($_GET['key']) ) && (isset($_GET['login']) && !empty($_GET['login']) ))
				{
					...
					if($email_verify_orignal_key == $email_verify_key)
					{
						...
						*/
						...
						{
							$this->pr_error_log("'The e-mail could not be sent. Possible reason: mail() function may have disabled by your host.'".($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
							$print_message = __("There was a problem and the email was probably not sent.",'pie-register');
							$type = "0";
						}
						...
						*/
						...
						if(empty($print_message)){
							$print_message = __("Please follow the link sent to your new Email to verify and make the change applied!","pie-register");
							$type="success";
						}
					}else{
						$print_message = __('Verification link is expired or invalid.','pie-register' );
						$type = "error";
					}
				}
				elseif( ($_GET['action'] == "email_edit" ) && (isset($_GET['key']) && !empty($_GET['key']) ) && (isset($_GET['login']) && !empty($_GET['login']) ))
				{
					...
					if($email_verify_orignal_key == $email_verify_key)
					{
						...
						if ( is_wp_error( $user_id_temp ) ) {
							$print_message = __('There is a problem updating your new Email Address, Pleasde try again!','pie-register' );
							$type = "error";
						}
						else{
							...
							$print_message = __("You have successfully verified your new Email address. Your Account Email has been changed","pie-register");
							$type = "success";
						}
					}
					else{
		...
		/*
			* PR Download User Files
		 */
		public function pieregister_download_user_files()
		{
			global $wpdb;
			$files 		= array();
			...
			if ( isset($context) )
			{
				if ($context == 'all')
				{
					...
					foreach( $users as $user ) 
					{
						...
						$uploads = $wpdb->get_results( $wpdb->prepare( $sql, array($user->ID) ) );
						
						if ( !empty($uploads) )
						{
							foreach( $uploads as $upload )
							{
								if ( $upload->meta_value != "" )
								{
									$files[] = $upload->meta_value;
								}
							}
						}
					}
				...
				if ( $context == 'user' )
				{
					if ( isset($id) )
					{
						...
						$uploads = $wpdb->get_results( $wpdb->prepare( $sql, array($id) ) );
			...
			{
				$zip = new ZipArchive();
				$zip_file = 'pie_files_'.time().'.zip';
				$zip->open($zip_file, ZipArchive::CREATE);
				foreach ( $files as $file ) 
				{
					$download_file = $this->read_file_from_url($file);
					$zip->addFromString(basename($file), $download_file);
				}

				$zip->close();

				$upload = wp_upload_bits($zip_file, null, file_get_contents($zip_file));
		...
		*/
		function piereg_payment_log_file_action(){
			...
			if( isset($_POST['piereg_download_payment_log_file']) && isset($_POST['piereg_payment_log']) && wp_verify_nonce( $_POST['piereg_payment_log'], 'piereg_wp_payment_log' ) ){
					...
					$this->pie_post_array['notice'] = __("Successfully remove payment log","pie-register");
