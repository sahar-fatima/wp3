...
class Registration_form_template extends Registration_form
{
	...
	function addUpload()
	{
		...
		if( isset( $this->field['file_size'] ) && !empty($this->field['file_size'])  )
		{
			$data .= '<p class="desc style_filetypes filesize">Allowed File Size: '.$this->field['file_size'].' KB</p>';	
		}
	...
	function addName()
	{
		...
		$data .= '<input value="'.$this->getDefaultValue('first_name').'" placeholder="'.$placeholder.'" data-field_id="'.$this->get_pr_widget_prefix().'piereg_field_'.$this->no.'" id="'.$this->id.'_firstname" name="first_name" class="'.$this->addClass().' input_fields piereg_name_input_field '.$cl_class.'"'.$cl_data.' '.$this->addValidation().'  type="text"  />';				
		...
		$data .= '<div class="fieldset">';
		if(!empty($label2)) $data .= '<label>'.$label2 . $field_required .'</label>';
		$data .= '<input value="'.$this->getDefaultValue('last_name').'" placeholder="'.$placeholder2.'" id="'.$this->id.'_lastname" name="last_name" class="'.$this->addClass().' input_fields piereg_name_input_field" '.$this->addValidation().'  type="text"  />';	
		$data .= $this->addDesc();
		$data .= '</div>';
		return $data;
			...
			if( isset($this->field['allow_payment_gateways']) && !empty($this->field['allow_payment_gateways']) && 'recurring' == 'enable' )
			{
				...
				*/
				if($this->field['field_as'] == 0){
					...
					$data .= '<div class="fieldset" >';
					$data .= $this->addLabel();
					$data .= '<select id="'.$field_id.'" name="pricing" data-field_id="piereg_field_'.$this->no.'" class="'.$this->addClass("input_fields").'" '.$this->addValidation().'  >';
					if(sizeof($this->field['display']) > 0)
					{	
						for($a = 0 ; $a < sizeof($this->field['display']) ; $a++)
						{
	...
	function addList()
	{
		...
		for($a = 1 ,$c=0; $a <= $this->field['rows'] ; $a++,$c++)
		{
			if($a==1)
			{
				...
				{
					$data .= ' <img src="'.plugins_url('pie-register').'/assets/images/plus.png" onclick="addList('.$this->field['rows'].','.$this->field['id'].');" alt="add" /></div>';		
				}
			...
			else
			{
				...
				for($b = 1 ; $b <= $this->field['cols'] ;$b++)
				{
					...
					$data .= ' <img src="'.plugins_url('pie-register').'/assets/images/minus.gif" onclick="removeList('.$this->field['rows'].','.$this->field['id'].','.$a.');" alt="add" />';
					$data .= '</div>';
			}
	...
	function addCheckRadio()
	{
		...
		if(sizeof($this->field['value']) > 0)
		{
			...
			for($a = 0 ; $a < sizeof($this->field['value']) ; $a++)
			{
				...
				$data .= '<div class="radio_conatainer">';
		 ...
		 if($this->field['address_type'] == "International")
		 {
			 ...
			 $data .= '<div  class="address2" >';
				$data .= '<select id="country_'.$this->id.'" name="'.$this->name.'[country]" class="'.$this->addClass().'" '.$this->addValidation().'>';
					$data .= '<option value="">'.__("Select Country","pie-register").'</option>';
						$data .= $options;
					$data .= '</select>';
	...
	function addDate()
	{
		...
		else if($this->field['date_type'] == "datepicker")
		{
				...
				{
					 $data .=  '<img id="'.$this->id.'_icon" class="calendar_icon" src="'.plugins_url('pie-register').'/assets/images/calendar.png" />';
				}
	...
	function addInvitationField(){
		...
		$invite_code = (isset($_GET['action'],$_GET['code']) && $_GET['action'] == 'ic' && !empty($_GET['code'])) ? trim($_GET['code']) : ""; 
		$data  = '<div class="fieldset">';
		$data .= $this->addLabel();
		$data .= '<input id="'.$this->id.'" name="invitation" class="'.$this->addClass().' '.$cl_class.'"'.$cl_data.' data-field_id="'.$this->get_pr_widget_prefix().'piereg_field_'.$this->no.'" placeholder="'.$this->field['placeholder'].'" type="text" value="'.$invite_code.'" />';
		if(!empty($invite_code))
		{
			$data .= '<input type="hidden" name="invitation_key" value="'.urldecode(trim($_GET['key'])).'">';
		}
		$data .= $this->addDesc();
		$data .= '</div>';
		return $data;
	...
	function addTermsField(){
		...
		$data .= sprintf(__('Click <a target="_blank" href="%s">here</a> to view.','pie-register'),$page_url);		
		$data .= '</label></div>';
		return $data;
	}
		...
		if(is_array($this->data)){
			foreach($this->data as $this->field)
			{
				...
				switch($this->field['type']){
					...
					break;
					case 'submit':
						do_action("pieregister_print_subscription_checkbox", $form_id);
