...
class Edit_form_template extends Edit_form
{
	...
	function addPassword(){
		...
		$data .= '</div></li><li class="fields pageFields_'.$this->pages.' '.$topclass.'"><div class="fieldset">';
	...
	function addProfilePic()
	{
		...
		$imgPath = (trim($val) != "")? $val : plugins_url("assets/images/userImage.png",dirname(dirname(__FILE__)));
		
			$data .= '<div class="edit-profile-img">';
				$data .= '<div class="file-wrapper"><img src="'.$imgPath.'" alt="'.__($imgPath,"pie-register").'" />';
	...
	function addName(){
		...
		$data .= '<input id="'.$this->id.'_firstname" data-field_id="piereg_field_'.$this->no.'" value="'.$val .'" placeholder="'.$this->field['placeholder'].'" name="first_name" class="'.$this->addClass().' input_fields piereg_name_input_field" '.$this->addValidation().' type="text"  />';
		...
		$data .= '<div class="fieldset">';
		if(!empty($this->field['label2'])) $data .= '<label>'.__($this->field['label2'],"pie-register").'</label>';
		$data .= '<input id="'.$this->id.'_lastname" value="'.$val .'" placeholder="'.$this->field['placeholder2'].'" name="last_name" class="'.$this->addClass().' input_fields piereg_name_input_field" '.$this->addValidation().' type="text"  />';
		$data .= $this->addDesc();
		//$data .= $this->made_conditional_logic();
		$data .= '</div>';
		return $data;
	...
	function editProfile($user){
		...
		if( is_array($this->data) && count($this->data) > 0 )
		{
			...
			foreach($this->data as $this->field)
			{
				...
				$profile_fields_data 	.= '<li class="fields '.$_parent.$topclass.' pageFields_'.$this->pages.' piereg_li_'.$class_x.'" '.$this->field_status.'>';
